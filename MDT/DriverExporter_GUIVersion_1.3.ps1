...
# - Auto resolve DeploymentShare
# - Test if PSDrive MDT exists, delete and remap
# - Clean up script / rename strings to better names
# - Recreate code to make it smaller (be a better coder)
# - Click Cancel in When choosing deployment share does not exit (fix in 1.4)
...
$Drivers = get-item $ParentPathOfDrivers
    ...
    if (!(Test-Path -path $DestinationPath$DriverPackage)) {New-Item $DestinationPath$DriverPackage -Type Directory}
...
# Inform that export is done!
[System.Windows.Forms.MessageBox]::Show('Export of ' + $DriverPackage.replace("\"," - ") + ' is done!')
...
$Exportbutton_OnClick= 
{
    ...
    if ($checkedListBox1.CheckedItems.Count -eq 0) {[System.Windows.Forms.MessageBox]::Show("Nothing Selected.Click cancel to close application or mark a driverpackage for export")}
    else{
